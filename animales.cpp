...
// Este metodo verificar que la comida que se pase como parametro este dentro de la opciones de dieta también de carnivoro, herbivoro y omnivoro.
...
void animales::modificarDieta(string accion, string comida){
    ...
    if(accion == "modificar" || accion == "Modificar"){
        this->mostrarDietasDisponoble(this->getTipoAlimentacion());
        cout <<endl<< "Ingrese la comida a cambiar por " << comida << " : ";
        getline(cin >> std::ws, nuevaComida);
        for(int i = 0; i < vectorDieta.size() && bandera == 0; i++){
            if(vectorDieta[i] == comida and verificarComida(nuevaComida)) {
...
void animales::mostrarDietaAnimal(){
    ...
    if(vectorDieta.size() != 0){
        for(int i = 0; i < vectorDieta.size(); i++){
            cout << "Comida: "<< vectorDieta[i] << endl;
        }
    }
    else{
...
//Este metodo es el que se encarga de que el usuario interactue con animal con un juego de adivinar un número.
void animales::jugar() {
    int num = -1;
    int intentos = 3;
...
// Este metodo se encarga de mandar al animal a dormir si se especifica el número de horas exacta.
...
// Este metodo se encarga de darle de comer al animal de acuerdo a la acción escogida por el usuario
void animales::comer(){
    string comida;
    if(vectorDieta.size() != 0){
        mostrarDietaAnimal();
